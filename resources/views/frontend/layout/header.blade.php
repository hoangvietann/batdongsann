<nav class="hidden relative lg:block bg-[#FAFAFA] border-b border-[#dfdfdf] " >
    <div class="flex justify-between items-center max-w-[1200px] mx-auto h-[40px]">
        <a href="#" class="flex items-center text-base text-[#828282] gap-2 font-bold" >
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M6.67962 3.32038L7.29289 2.70711C7.68342 2.31658 8.31658 2.31658 8.70711 2.70711L11.2929 5.29289C11.6834 5.68342 11.6834 6.31658 11.2929 6.70711L9.50048 8.49952C9.2016 8.7984 9.1275 9.255 9.31653 9.63307C10.4093 11.8186 12.1814 13.5907 14.3669 14.6835C14.745 14.8725 15.2016 14.7984 15.5005 14.4995L17.2929 12.7071C17.6834 12.3166 18.3166 12.3166 18.7071 12.7071L21.2929 15.2929C21.6834 15.6834 21.6834 16.3166 21.2929 16.7071L20.6796 17.3204C18.5683 19.4317 15.2257 19.6693 12.837 17.8777L11.6286 16.9714C9.88504 15.6638 8.33622 14.115 7.02857 12.3714L6.12226 11.163C4.33072 8.7743 4.56827 5.43173 6.67962 3.32038Z" fill="#EB5757"/>
            </svg>
            Hotline: 0973997777
        </a>   
        
        <div class="flex item-center h-full" >
            <a href="{{ route('posts.create') }}" class="flex items-center text-base gap-2 text-[#828282] px-4 border-r hover:text-black">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <mask id="path-1-outside-1_1_46" maskUnits="userSpaceOnUse" x="3" y="4" width="17" height="17" fill="black">
            ...
            @if(auth()->check())
                <div class="relative group">
                    ...
                    <div class="flex items-center h-full pl-4">
                        <button onclick="toggleUserDropdown()" class="flex items-center gap-2 group-hover:text-gray-900 focus:outline-none">
                            <span class="text-base text-[#828282] font-bold">{{ auth()->user()->name }}</span>
                            <img src="{{ isset(auth()->user()->avatar) ? asset('uploads/'.auth()->user()->avatar) : asset('dist/images/profile-1.jpg') }}" alt="avatar" class="rounded-full w-10 h-10 object-cover">
                        </button>                        
                    </div>
                    <ul id="menu" class="absolute hidden right-0 w-[315px] bg-white mt-2 space-y-1 text-black py-2 rounded-lg shadow-xl z-[9999]">
                        <li>
                            <a href="{{ route('posts.my-posts') }}" class="flex items-center gap-2 px-4 py-3 hover:bg-gray-200">
                                <i class="fa-solid fa-list"></i>
                                Quản lý bài đăng
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('profile.index') }}" class="flex items-center gap-2 px-4 py-3 hover-bg-gray-100 hover:bg-gray-200">
                                <i class="fa-solid fa-user"></i>
                                Thông tin cá nhân
                            </a>
                        </li>
                        <li>
                            <a href="{{ route('change-password') }}" class="flex items-center gap-2 px-4 py-3 hover-bg-gray-100 hover:bg-gray-200">
                        ...
                        <li class="border-t">
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit" class="flex items-center gap-2 px-4 py-3 hover:bg-gray-200">
            ...
            @else
                <a href="{{ route('login') }}" class="flex items-center text-base h-full px-4 gap-2 border-r text-[#828282] hover:text-black">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M19.7274 20.4471C19.2716 19.1713 18.2672 18.0439 16.8701 17.2399C15.4729 16.4358 13.7611 16 12 16C10.2389 16 8.52706 16.4358 7.12991 17.2399C5.73276 18.0439 4.72839 19.1713 4.27259 20.4471" stroke="#EB5757" stroke-width="2" stroke-linecap="round"/>
                        <circle cx="12" cy="8" r="4" stroke="#EB5757" stroke-width="2" stroke-linecap="round"/>
                    </svg>
                    Đăng nhập 
                </a>  
                <a href="{{ route('register') }}" class="flex items-center gap-2 pl-4 text-base text-[#828282] hover:text-black">
                    <svg width="24" height="26" viewBox="0 0 24 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <ellipse cx="9" cy="15.0308" rx="4" ry="4.29451" stroke="#EB5757" stroke-width="2"/>
                        <path d="M12 11.8099L15.5 8.05221M17 6.44177L15.5 8.05221M15.5 8.05221L18 10.7363" stroke="#EB5757" stroke-width="2" stroke-linecap="round"/>
                    </svg>
                    Đăng ký
                </a>              
            @endif
