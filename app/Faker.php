...
class Faker
{
    ...
    public static function fakePosts(){
        ...
        // Tạo 10 bài đăng giả lập
        for ($i = 0; $i < 10; $i++) {
            $name_user = $faker->name;
            $name_post_category = $faker->word;
            $title = $faker->sentence;
            $price = $faker->randomFloat(2, 10, 1000); // Giá từ 10 đến 1000 với 2 chữ số sau dấu thập phân
            $area = $faker->randomFloat(2, 50, 500); // Diện tích từ 50 đến 500 với 2 chữ số sau dấu thập phân
            $status = $faker->randomElement(['active', 'inactive', 'draft']);
            $posts[] = [
                'name_user' => $name_user,
                'name_post_category' => $name_post_category,
                'title' => $title,
                'price' => $price,
                'area' => $area,
                'status' => $status,
    ...
    public static function fakeNews1()
    {
        $faker = \Faker\Factory::create();

        $news = [];

        // Tạo 10 bài tin tức giả lập
        for ($i = 0; $i < 10; $i++) {
            $user_name = $faker->name;
            $name_news_category = $faker->word;
            $title = $faker->sentence;
